...
public class DropValveManager : MonoBehaviour
{
    ...
    public void CreateValvesCustom(uint spawnNum, Vector3 spawnPosition, bool isAuto, float cspeed, float upVelocity)
    {
        GameObject obj = Instantiate(valveEmitter, spawnPosition, Quaternion.identity);
        obj.transform.SetParent(transform);
        if (spawnNum > 36)
        {
            obj.GetComponent<ValveEmitter>().Create(spawnNum, cspeed, 2.0f, isAuto, upVelocity);
        }
        else if (spawnNum > 24)
        {
            obj.GetComponent<ValveEmitter>().Create(spawnNum, cspeed, 1.5f, isAuto, upVelocity);
        }
        else if (spawnNum > 12)
        {
            obj.GetComponent<ValveEmitter>().Create(spawnNum, cspeed, 1.0f, isAuto, upVelocity);
        }
        else
        {
            obj.GetComponent<ValveEmitter>().Create(spawnNum, cspeed, 0.5f, isAuto, upVelocity);
        }
    }
}
